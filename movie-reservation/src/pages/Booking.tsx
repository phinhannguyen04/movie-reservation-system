...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, MapPin, Clock, Info, CheckCircle2 } from 'lucide-react';
...
export function Booking() {
  ...
  // Auto-select showtime if time and cinema are provided in URL
  useEffect(() => {
    if (initialTimeParam && initialCinemaParam && showtimes.length > 0 && !selectedShowtime) {
      ...
      if (foundShowtime) {
        ...
        // Auto-advance to step 2 (Select Seats) for a seamless experience
        setStep(2);
      }
    }
  ...
  const handleConfirmBooking = () => {
    ...
    const newBookingId = `B-${Math.floor(Math.random() * 100000000).toString().padStart(8, '0')}`;
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="bg-surface rounded-xl p-6 border border-white/5 overflow-hidden">
                          ...
                          <div className="flex gap-2">
                            {seats.filter(s => s.row === row).map(seat => {
                              ...
                              if (seat.status === 'occupied') {
                                seatClass = cn(seatClass, "bg-seat-occupied text-transparent cursor-not-allowed opacity-50");
                              } else if (isSelected) {
                                seatClass = cn(seatClass, "bg-seat-selected text-white shadow-[0_0_10px_rgba(229,9,20,0.5)] scale-110");
                              } else {
                                if (seat.type === 'vip') {
                                  seatClass = cn(seatClass, "bg-seat-available border border-seat-vip/50 text-seat-vip hover:bg-seat-vip/20 cursor-pointer");
                                } else if (seat.type === 'couple') {
                                  seatClass = cn(seatClass, "bg-seat-available border border-seat-couple/50 text-seat-couple hover:bg-seat-couple/20 cursor-pointer w-[72px]");
                                } else {
                                  seatClass = cn(seatClass, "bg-seat-available border border-white/10 text-gray-400 hover:bg-white/10 cursor-pointer");
                                }
                              }
                  ...
                  <div className="flex flex-wrap justify-center gap-6 pt-6 border-t border-white/10">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-t bg-seat-available border border-white/10" />
                      <span className="text-xs text-gray-400">Available</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-t bg-seat-selected shadow-[0_0_8px_rgba(229,9,20,0.5)]" />
                      <span className="text-xs text-gray-400">Selected</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-t bg-seat-occupied opacity-50" />
                      <span className="text-xs text-gray-400">Occupied</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-t bg-seat-available border border-seat-vip/50" />
                      <span className="text-xs text-gray-400">VIP ($15)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-11 h-5 rounded-t bg-seat-available border border-seat-couple/50" />
                      <span className="text-xs text-gray-400">Couple ($25)</span>
                    </div>
                  </div>
                </div>
              </motion.div>
                ...
                <div className="bg-surface rounded-xl p-6 border border-white/5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    ...
                    <button className="p-4 rounded-lg border border-white/10 bg-background hover:border-white/30 flex items-center justify-between transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-[#00457C] rounded flex items-center justify-center">
                          <span className="text-white font-bold text-xs italic">Pay</span>
                        </div>
            ...
            {step === 4 && (
              ...
              >
                ...
                <p className="text-gray-400 text-center mb-8">Your ticket has been sent to your email.</p>
                  ...
                  <div className="p-6 bg-white text-black relative">
                    ...
                    <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-6">
                      <div>
                        ...
                        <p className="font-bold">{format(selectedDate, 'dd MMM yyyy')}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-bold">{selectedShowtime?.screen}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 uppercase">Seats</p>
                        <p className="font-bold text-primary">{selectedSeats.map(s => s.id).join(', ')}</p>
                      </div>
                    </div>
                ...
                <div className="mt-8 flex gap-4">
                  <button 
                    onClick={() => navigate('/')}
                    className="px-6 py-3 bg-surface hover:bg-surface-light text-white font-medium rounded-lg transition-colors"
                  >
                    Back to Home
                  </button>
                  <button 
            ...
            <div className="bg-surface rounded-xl p-6 border border-white/5 sticky top-24">
              ...
              <div className="space-y-4 text-sm mb-6 pb-6 border-b border-white/10">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-right max-w-[150px] truncate">
                    {selectedCinema ? cinemas.find(c => c.id === selectedCinema)?.name : '-'}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{selectedShowtime ? selectedShowtime.time : '-'}</span>
                </div>
                <div className="flex justify-between">
              ...
              {step === 1 && (
                <button
                  ...
                  onClick={() => setStep(2)}
                  className="w-full py-3 bg-primary hover:bg-primary-hover disabled:bg-surface-light disabled:text-gray-500 disabled:cursor-not-allowed text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
                >
                  Select Seats
              ...
              {step === 3 && (
                <button
                  ...
                  className="w-full py-3 bg-primary hover:bg-primary-hover text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(229,9,20,0.4)]"
                >
